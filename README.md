...
- **New button, guided**: + Button → New button… asks what the button should do. *Go to a
  frequency* is preselected and filled in from where the radio is now, with mode and antenna lists
  the radio itself reports, so Next then Save makes a working button. The other choices: remember
  what the radio is doing now, change band, a common action (switch antenna, move TX, pick or close
  a slice, set power), or write the commands yourself. Page two names it and shows the commands it
  will send. No command knowledge needed.
- **Editor** for an existing button (right-click → Edit), or from the guided start via Edit
  commands…: label, hotkey, colour, row, and the command list with Capture, Insert example and the
  Reference beside it.
...
- **Runs on**: every button runs on slice A unless you say otherwise, so your memories still land on
  ...
  "whichever slice is active"; a button made while B is active is set to B for you. A small mark in
  the corner shows the exceptions (a letter, or "act"), and + Button can switch the whole grid at once.
- **Placeholders**: `{slice}` is the button's slice (A unless Runs on says otherwise), `{tx}` the transmit slice, `{A}`..`{H}` a slice
...
A new button starts with a question. The first choice is filled in from where the radio is now.

![New button: what should it do](docs/screenshots/newbutton.png)

Page two names it and shows what it will send.

![New button: name it](docs/screenshots/newbutton-name.png)

The editor, for an existing button or for writing commands by hand.

![Button editor](docs/screenshots/editor.png)
